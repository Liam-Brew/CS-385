...
Node *lowest_common_ancestor(Node *p, Node *q) {
    ...
    if (p == q) return p;
    if (p->level > q->level) return lowest_common_ancestor(p->parent, q);
    if (p->level < q->level) return lowest_common_ancestor(p, q->parent);
...
Node *create_bst_helper(int data[], int low, int high, int level) {
    ...
    parent->right = create_bst_helper(data, mid, high, level + 1);
...
bool is_bst(Node *n, int *min, int *max) {
    ...
    if((min != nullptr && n->data <= *min) || (max != nullptr && n->data >= *min)){
        return false;
    }
