...
vector<vertex_id> top_sort(const Graph* const graph) {
    ...
    size_t num_verticies = static_cast<size_t>(graph->get_highest_vertex_id());
    order.reserve(num_verticies);
    int* in_degrees = new int[num_verticies + 1];
    fill(in_degrees, in_degrees + num_verticies + 1, -1);
    for (auto it = vertices.begin(); it != vertices.end(); ++it) {
        in_degrees[*it] = 0;
    }
    priority_queue<vertex_id, vector<vertex_id>, greater<vertex_id>> zero_indegree_nodes;

    for (size_t i = 1; i <= num_verticies; ++i) {
